...
public class Vector {
    ...
    public static double[][] addVecToSet(
            double[][] vecSet,
            double[] addVec) {
        int m = com.example.HealthMonitor.Math.Matrix.getNumOfRows(vecSet);
        int n = com.example.HealthMonitor.Math.Matrix.getNumOfColumns(vecSet);
        double[][] res = com.example.HealthMonitor.Math.Matrix.newMatrix(m, n);
        for (int i = 0; i < m; ++i) {
            double add = addVec[i];
            for (int j = 0; j < n; ++j) {
